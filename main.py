...
class EmailDetector:
    ...
    def _evaluate_security_posture(self, records: DNSRecords, current_score: int, reasons: List[str], is_managed_routing: bool, is_self_hosted: bool) -> int:
        ...
        has_spf, non_spf_count, is_deny_all = self._parse_spf(records.txt)
        ...
        if is_deny_all and len(records.mx) > 0:
            score_delta += 10
            reasons.append("Receive-only configuration (SPF -all)")
    ...
    def _analyze_managed_patterns(self, records: DNSRecords, extracted: Any, base_domain: str, reasons: List[str], 
        ...
        _, non_spf_count, _ = self._parse_spf(records.txt)
        is_major_ns = any(any(p in ns.lower() for p in ScoringConfig.MAJOR_NS_PROVIDERS) for ns in records.ns)
        ...
        has_spf, _, _ = self._parse_spf(records.txt)
    ...
    def _parse_spf(self, records: List[str]) -> Tuple[bool, int, bool]:
        has_spf = False
        non_spf_count = 0
        is_deny_all = False
        
        for txt in records:
            txt_l = txt.lower().strip('"').strip()
            if txt_l.startswith('v=spf1'):
                has_spf = True
                parts = txt_l.split()
                if '-all' in parts:
                    has_other_mech = False
                    for p in parts:
                        if p.startswith('v=spf1') or p == '-all': continue
                        mech = p.lstrip('+-~?')
                        if any(mech.startswith(m) for m in ['a', 'mx', 'include', 'ip4', 'ip6', 'ptr', 'exists', 'redirect']):
                            has_other_mech = True
                            break
                    
                    if not has_other_mech:
                        is_deny_all = True

            elif any(word in txt_l for word in ScoringConfig.VERIFICATION_TAGS):
                non_spf_count += 1
        return has_spf, non_spf_count, is_deny_all
